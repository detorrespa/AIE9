...
class BasePrompt:
    ...
    def format_prompt(self, **kwargs) -> str:
        ...
        format_dict = {
            var: merged_kwargs.get(var, self.defaults.get(var, ""))
            for var in variables
        }
...
class PromptTemplate(BasePrompt):
    ...
    def compose(
        self, *templates: "PromptTemplate", separator: str = "\n\n"
    ) -> "PromptTemplate":
        """
        ...
        return PromptTemplate(
            combined_prompt, strict=self.strict, defaults=combined_defaults
        )
...
if __name__ == "__main__":
    ...
    prompt_with_defaults = BasePrompt(
        "Hello {name}, you are {age} years old", defaults={"age": 25}
    )
    ...
    system = SystemRolePrompt(
        "You are a helpful assistant", defaults={"tone": "friendly"}
    )
